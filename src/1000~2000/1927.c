...
static int heap_pop_min(int* const pa_heap, int* const out_size);
...
int main(void)
{
    ...
    for (i = 0; i < n_ops; ++i) {
        ...
        if (x != 0) {
            ...
            int result = heap_pop_min(pa_heap, &heap_size);
            printf("%d\n", result);
        }
    }
...
static int heap_pop_min(int* const pa_heap, int* const out_size)
{
    ...
    *out_size = *out_size - 1;
    ...
    while (1) {
        ...
        if (pa_heap[index] <= pa_heap[smaller_child]) {
            break;
        }

        swap_int(&pa_heap[index], &pa_heap[smaller_child]);
        index = smaller_child;
    }
